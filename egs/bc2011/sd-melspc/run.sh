...
#            OHTER SETTING            #
#######################################
BLIZZARD2011_DB_ROOT=database/blizzard2011
tag=softmax
...
if echo ${stage} | grep -q 0; then
    ...
    find ${BLIZZARD2011_DB_ROOT}/wav_train -name "*.wav" \
        | sort > data/${train}/wav.scp
    [ ! -e data/${eval} ] && mkdir -p data/${eval}
    find ${BLIZZARD2011_DB_ROOT}/wav_test -name "*.wav" \
        | sort > data/${eval}/wav.scp
fi
# }}}
...
# set variables
if [ ! -n "${tag}" ];then
    expdir=exp/tr_bc2011_16k_sd_${feature_type}_nq${n_quantize}_na${n_aux}_nrc${n_resch}_nsc${n_skipch}_ks${kernel_size}_dp${dilation_depth}_dr${dilation_repeat}_lr${lr}_wd${weight_decay}_bl${batch_length}_bs${batch_size}
    if ${use_noise_shaping};then
        expdir=${expdir}_ns
    fi
...
if echo ${stage} | grep -q 6 && ${use_noise_shaping}; then
    ...
    ${train_cmd} --num-threads ${n_jobs} exp/noise_shaping/noise_shaping_mcep_${eval}.log \
        noise_shaping.py \
            --waveforms data/${gen_set}/wav_generated.scp \
            --stats data/${train}/stats.h5 \
